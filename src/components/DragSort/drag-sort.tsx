...
export class DragSortComponent {
  ...
  initSortable() {
    try {
      ...
      // console.log("sortttt watch1", this.sortable, this.sort);

      const el: HTMLElement = this.el.querySelector(
        ".saki-drag-sort-component",
      );
      if (this.sort && el) {
        // console.log("sortttt watch2", this.sortable, this.sort, el);
        this.sortable = new Sortable(el, {
          ...
          onEnd: (e) => {
            console.log(e);
            const { oldIndex, newIndex, from } = e;

            // ================= 【核心修复：手动还原 DOM 状态】 =================
            // 判断索引是否发生了有效改变（防止原地点击也触发还原）
            if (
              oldIndex !== undefined &&
              newIndex !== undefined &&
              oldIndex !== newIndex
              ...
              if (newIndex > oldIndex) {
                // 如果往下拖拽：把当前拖拽的元素，插回到它原来位置的那个元素【之前】
                from.insertBefore(e.item, children[oldIndex]);
              } else {
                // 如果往上拖拽：把当前拖拽的元素，插回到它原来位置的下一个元素【之前】
                // 如果已经是最后一项，则传入 null，相当于 appendChild
                from.insertBefore(e.item, children[oldIndex + 1] || null);
              }
            }
        ...
        // console.log("sortttt final", this.sortable, this.sort);
      }
      // this.sortable.destroy()
